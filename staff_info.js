...
const app = Vue.createApp({
    data() {
        return {
            staff_id : sessionStorage.getItem('staff_lookup_id'),
            staff: {},
            manager: {},
            p_role: {},
            s_roles: [],
    ...
    created(){
        $(async()=>{
            // Get staff info
            var serviceURL1 = 'http://localhost:5000/staff/'+this.staff_id;
            try {
                const response =
                    await fetch(
            ...
            // Get manager info
            var serviceURL2 = 'http://localhost:5000/staff/officerofstaff/'+this.staff_id;
            try {
                const response2 =
                    await fetch(
            ...
            var serviceURL3 = 'http://localhost:5000/staff/p_role/'+this.staff_id;
            try {
                const response3 =
                    await fetch(
            ...
            var serviceURL5 = 'http://localhost:5000/staff/s_role/'+this.staff_id;
            try {
                const response5 =
                    await fetch(
                ...
                if (response5.status == 200){
                    var s_roles = result5.data.secondary_roles
                }
            }
            catch (error) {
                console.log('Error in processing the request.')
            }
            for (i=0; i<s_roles.length; i++){
                var serviceURL6 = 'http://localhost:5001/role/'+s_roles[i].RoleID;
                try {
                    const response6 =
                        await fetch(
            ...
            // Get staff skills
            var serviceURL7 = 'http://localhost:5000/staff/skillsofstaff/'+this.staff_id;
            try {
                const response7 =
                    await fetch(
